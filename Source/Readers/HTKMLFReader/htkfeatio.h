...
public:
    ...
    public:
        ...
        {
            return Microsoft::MSR::CNTK::ToWString(logicalpath, false);
        }
...
#ifdef _MSC_VER
        wstring key = Microsoft::MSR::CNTK::ToWString(regex_replace(filename, regex("\\.[^\\.\\\\/:]*$"), string()), false); // delete extension (or not if none)
#else
        wstring key = Microsoft::MSR::CNTK::ToWString(msra::dbn::removeExtension(filename), false); // note that c++ 4.8 is incomplete for supporting regex
#endif
