...
File::File(const std::string& filename, int fileOptions)
{
    ...
    Init(Microsoft::MSR::CNTK::ToWString(filename, false).c_str(), fileOptions);
}
...
/*static*/ wstring File::GetExecutablePath()
{
    ...
    if (readlink(path, dest, PATH_MAX) == -1)
        ...
        return Microsoft::MSR::CNTK::ToWString(dest, false);
#endif
}
...
static void PushBackString(vector<wstring>& lines, string& s)       { lines.push_back(Microsoft::MSR::CNTK::ToWString(s, false)); }
...
FARPROC Plugin::LoadInternal(const std::wstring& plugin, const std::string& proc, bool isCNTKPlugin)
{
    ...
    if(!boost::ends_with(m_dllName, L".pyd"))
    {
        if (isCNTKPlugin)
        {
            ...
            m_dllName += L"-" + Microsoft::MSR::CNTK::ToWString(CNTK_COMPONENT_VERSION, false);
        }
...
void* Plugin::LoadInternal(const std::string& plugin, const std::string& proc, bool isCNTKPlugin)
{
    ...
    wstring soNameW = Microsoft::MSR::CNTK::ToWString(plugin, false);
