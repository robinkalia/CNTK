        ...
        else // One or more config file paths specified in a "+"-separated list.
        {
            ...
            for (auto filePath : filePathsVec)
            {
                if (std::find(resolvedConfigFiles.begin(), resolvedConfigFiles.end(), filePath) == resolvedConfigFiles.end())
                {
                    ...
                    GetBrainScriptNetworkBuilderIncludePaths().push_back(File::DirectoryPathOf(Microsoft::MSR::CNTK::ToWString(filePath, false)));
                }
                else
...
{
    return ReadConfigFiles(Microsoft::MSR::CNTK::ToWString(filePaths, false));
}
...
{
    return ReadConfigFile(Microsoft::MSR::CNTK::ToWString(filePath, false));
}
