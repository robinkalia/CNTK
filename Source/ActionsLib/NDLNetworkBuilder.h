...
public:
...

    {
        if (pass > ndlPassInitial && evaluateNode)
        {
            ...
            std::wstring wname = Microsoft::MSR::CNTK::ToWString(name, false);
            if (nodeParam->GetType() == ndlTypeDotParameter)
            {
                ...
                if (resolvedParam != nullptr && resolvedParam->GetType() == ndlTypeMacroCall)
                {
                    ...
                    std::wstring resolvedParamName = Microsoft::MSR::CNTK::ToWString(resolvedParam->GetName(), false);
                    wname = baseName.empty() ? resolvedParamName + L"." + Microsoft::MSR::CNTK::ToWString(nameAfterDot, false) : baseName + L"." + resolvedParamName + L"." + Microsoft::MSR::CNTK::ToWString(nameAfterDot, false);
                }
    ...
    virtual NDLNode<ElemType>* EvaluateParameter(NDLNode<ElemType>* node, NDLNode<ElemType>* nodeParam, const std::wstring& baseNameP, const NDLPass pass)
    {
        ...
        if (script == NULL)
        {
            std::wstring name = baseName + L"." + Microsoft::MSR::CNTK::ToWString(node->GetName(), false);
    ...
    virtual void Init(const ConfigParameters& config)
    {
        ...
        if (config.Exists("ndlMacros"))
        {
            ...
            if (!ndlMacrosPaths.empty())
            {
                ...
                {
                    ndlScript.LoadConfigFileAndResolveVariables(Microsoft::MSR::CNTK::ToWString(filePath, false), config);
                }
            }
        }
