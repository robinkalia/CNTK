...
void NDLNodeEvaluatorImpl<ElemType>::Evaluate(NDLNode<ElemType>* node, const wstring& baseName, const NDLPass pass)
{
    ...
    std::wstring name = Microsoft::MSR::CNTK::ToFixedWString(node->GetName(), false);
    if (!baseName.empty())
    {
        name = baseName + L"." + name;
    }

    std::wstring cnNodeType = Microsoft::MSR::CNTK::ToFixedWString(node->GetValue(), false);
    ...
    else if (OperationNameOf(LearnableParameter) == cnNodeType || cnNodeType == L"ImageParameter")
    {
        ...
        else if (pass == ndlPassFinal)
        {
            ...
            else if (EqualCI(initString, L"fromFile"))
            {
                ...
                dynamic_pointer_cast<LearnableParameter<ElemType>>(nodePtr)->InitFromFile(Microsoft::MSR::CNTK::ToFixedWString(initFromFilePath, false));
            }
